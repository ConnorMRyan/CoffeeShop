...
def main() -> None:
    ...
    for t in range(t_cfg.total_steps):
        ...
        for aid, actor in actors.items():
            ...
            # Track inactivity penalty per agent
            # OvercookedSocialWrapper applies penalty to both reward and sparse_reward.
            # We want to log the total penalty per episode.
            # The info dict from wrapper has "inactivity_penalty" (sum for both agents in that env)
            # but we need it per agent if we want granular logging.
            # Wait, the wrapper info contains "total_inactivity_penalty" which is accumulated.
            # However, for VectorSocialRunner, it's easier to just accumulate here.
            # We don't have per-agent penalty in info yet, only step-total.
            # Let's assume we want to log the step-total per env or just the global total.
            # User said "total penalty applied per episode ... as env/inactivity_penalty".
            # If multiple envs are running, we can average the per-episode totals.
        metrics.update({
        ...
        if not t_cfg.get("baseline", False):
            if (t + 1) % t_cfg.critic_update_interval == 0 and len(mediator._buffer) >= 64:
                ...
                # FIX (Detective personality): c_loss is now a mediator metric only.
                # No longer overwriting each actor._last_critic_loss here.
